...
export const CellActionColor = (props: Props) => {
  const { data } = props
  ...
  const { mutate: deleteColor } = useMutation({
    mutationFn: () =>
      axios.delete(`/api/store/${data.storeId}/color/${data.id}`),
    ...
    <>
      <AlertModal
        ...
        message={
          <>
            ...
            <span className="italic">{data.name}</span> color?
          </>
        }
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          ...
          <DropdownMenuItem
            onClick={() => router.push(`/${params.storeName}/color/${data.id}`)}
          >
